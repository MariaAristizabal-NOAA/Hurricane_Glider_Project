...
def depth_aver_100m_to_300m(depth,var):

    varmean = np.empty(var.shape[1])
    varmean[:] = np.nan
    if depth.ndim == 1:
        okd = np.logical_and(np.abs(depth) <= 300,np.abs(depth) >= 100)
        if len(depth[okd]) != 0:
            for t in np.arange(var.shape[1]):
                if len(np.where(np.isnan(var[okd,t]))[0])>10:
                    varmean[t] = np.nan
                else:
                    varmean[t] = np.nanmean(var[okd,t],0)
    else:
        for t in np.arange(depth.shape[1]):
            okd = np.logical_and(np.abs(depth[:,t]) <= 300,np.abs(depth[:,t]) >= 100)
            if len(depth[okd,t]) != 0:
                if len(np.where(np.isnan(var[okd,t]))[0])>10:
                    varmean[t] = np.nan
                else:
                    varmean[t] = np.nanmean(var[okd,t])
            else:
                varmean[t] = np.nan
...
for id in gliders:
    empty_dataset = check_dataset_empty(url_erddap,id,date_ini,date_end,lon_lim,lat_lim)
...
#%%
DF_RTOFS_temp_salt_shelf = pd.DataFrame()
DF_RTOFS_DA_temp_salt_shelf = pd.DataFrame()
DF_GOFS_temp_salt_shelf = pd.DataFrame()
DF_RTOFS_MLD_shelf = pd.DataFrame()
DF_RTOFS_DA_MLD_shelf = pd.DataFrame()
DF_GOFS_MLD_shelf = pd.DataFrame()
DF_RTOFS_OHC_shelf = pd.DataFrame()
DF_RTOFS_DA_OHC_shelf = pd.DataFrame()
DF_GOFS_OHC_shelf = pd.DataFrame()
DF_RTOFS_T100_shelf = pd.DataFrame()
DF_RTOFS_DA_T100_shelf = pd.DataFrame()
DF_GOFS_T100_shelf = pd.DataFrame()
DF_RTOFS_T100_300_shelf = pd.DataFrame()
DF_RTOFS_DA_T100_300_shelf = pd.DataFrame()
DF_GOFS_T100_300_shelf = pd.DataFrame()
DF_RTOFS_PEA_shelf = pd.DataFrame()
DF_RTOFS_DA_PEA_shelf = pd.DataFrame()
DF_GOFS_PEA_shelf = pd.DataFrame()

DF_RTOFS_temp_salt_cp = pd.DataFrame()
DF_RTOFS_DA_temp_salt_cp = pd.DataFrame()
DF_GOFS_temp_salt_cp = pd.DataFrame()
DF_RTOFS_MLD_cp = pd.DataFrame()
DF_RTOFS_DA_MLD_cp = pd.DataFrame()
DF_GOFS_MLD_cp = pd.DataFrame()
DF_RTOFS_OHC_cp = pd.DataFrame()
DF_RTOFS_DA_OHC_cp = pd.DataFrame()
DF_GOFS_OHC_cp = pd.DataFrame()
DF_RTOFS_T100_cp = pd.DataFrame()
DF_RTOFS_DA_T100_cp = pd.DataFrame()
DF_GOFS_T100_cp = pd.DataFrame()
DF_RTOFS_T100_300_cp = pd.DataFrame()
DF_RTOFS_DA_T100_300_cp = pd.DataFrame()
DF_GOFS_T100_300_cp = pd.DataFrame()
DF_RTOFS_PEA_cp = pd.DataFrame()
DF_RTOFS_DA_PEA_cp = pd.DataFrame()
DF_GOFS_PEA_cp = pd.DataFrame()

DF_RTOFS_temp_salt_sp = pd.DataFrame()
DF_RTOFS_DA_temp_salt_sp = pd.DataFrame()
DF_GOFS_temp_salt_sp = pd.DataFrame()
DF_RTOFS_MLD_sp = pd.DataFrame()
DF_RTOFS_DA_MLD_sp = pd.DataFrame()
DF_GOFS_MLD_sp = pd.DataFrame()
DF_RTOFS_OHC_sp = pd.DataFrame()
DF_RTOFS_DA_OHC_sp = pd.DataFrame()
DF_GOFS_OHC_sp = pd.DataFrame()
DF_RTOFS_T100_sp = pd.DataFrame()
DF_RTOFS_DA_T100_sp = pd.DataFrame()
DF_GOFS_T100_sp = pd.DataFrame()
DF_RTOFS_T100_300_sp = pd.DataFrame()
DF_RTOFS_DA_T100_300_sp = pd.DataFrame()
DF_GOFS_T100_300_sp = pd.DataFrame()
DF_RTOFS_PEA_sp = pd.DataFrame()
DF_RTOFS_DA_PEA_sp = pd.DataFrame()
DF_GOFS_PEA_sp = pd.DataFrame()
...
for f,dataset_id in enumerate(gliders):
    ...
    if not empty_dataset:
        ...
        #%% Calculate T100 and T100_300
        # glider
        T100_glid = depth_aver_top_100(depthg_gridded,tempg_gridded)
        T100_300_glid = depth_aver_100m_to_300m(depthg_gridded,tempg_gridded)

        # RTOFS
        T100_RTOFS = depth_aver_top_100(depth_RTOFS,target_temp_RTOFS)
        T100_300_RTOFS = depth_aver_100m_to_300m(depth_RTOFS,target_temp_RTOFS)

        # RTOFS-DA
        T100_RTOFS_DA = depth_aver_top_100(depth_RTOFS_DA,target_temp_RTOFS_DA)
        T100_300_RTOFS_DA = depth_aver_100m_to_300m(depth_RTOFS_DA,target_temp_RTOFS_DA)

        # GOFS
        T100_GOFS = depth_aver_top_100(depth_GOFS,target_temp_GOFS)
        T100_300_GOFS = depth_aver_100m_to_300m(depth_GOFS,target_temp_GOFS)
        ...
        T100_300_glid_to_RTOFS = T100_300_glid[oktimeg_rtofs]
        ...
        T100_300_glid_to_RTOFS_DA = T100_300_glid[oktimeg_rtofs_da]
        ...
        T100_300_glid_to_GOFS = T100_300_glid[oktimeg_gofs]
        ...
        df_RTOFS_T100_300 = pd.DataFrame(data=np.array([T100_300_glid_to_RTOFS,T100_300_RTOFS]).T,\
                          columns=['T100_300_obs','T100_300_RTOFS'])

        #%% Define dataframe
        df_RTOFS_DA_T100 = pd.DataFrame(data=np.array([T100_glid_to_RTOFS_DA,T100_RTOFS_DA]).T,\
                          columns=['T100_obs','T100_RTOFS_DA'])
        df_RTOFS_DA_T100_300 = pd.DataFrame(data=np.array([T100_300_glid_to_RTOFS_DA,T100_300_RTOFS_DA]).T,\
                          columns=['T100_300_obs','T100_300_RTOFS_DA'])

        #%% Define dataframe
        df_GOFS_T100 = pd.DataFrame(data=np.array([T100_glid_to_GOFS,T100_GOFS]).T,\
                          columns=['T100_obs','T100_GOFS'])
        df_GOFS_T100_300 = pd.DataFrame(data=np.array([T100_300_glid_to_GOFS,T100_300_GOFS]).T,\
                          columns=['T100_300_obs','T100_300_GOFS'])
        ...
        if np.logical_or.reduce([dataset_id.split('-')[0]=='amelia',dataset_id.split('-')[0]=='blue',\
                                 dataset_id.split('-')[0]=='franklin',dataset_id.split('-')[0]=='maracoos',\
                                 dataset_id.split('-')[0][0:2]=='ng',dataset_id.split('-')[0][0:2]=='ru',\
                                 dataset_id.split('-')[0]=='sylvia',dataset_id.split('-')[0][0:2]=='ud']):
            print('Shelf glider: '+dataset_id)

            #%% Concatenate data frames
            DF_RTOFS_temp_salt_shelf = pd.concat([DF_RTOFS_temp_salt_shelf, df_RTOFS_temp_salt])
            DF_RTOFS_DA_temp_salt_shelf = pd.concat([DF_RTOFS_DA_temp_salt_shelf, df_RTOFS_DA_temp_salt])
            DF_GOFS_temp_salt_shelf = pd.concat([DF_GOFS_temp_salt_shelf, df_GOFS_temp_salt])
            DF_RTOFS_MLD_shelf = pd.concat([DF_RTOFS_MLD_shelf, df_RTOFS_MLD])
            DF_RTOFS_DA_MLD_shelf = pd.concat([DF_RTOFS_DA_MLD_shelf, df_RTOFS_DA_MLD])
            DF_GOFS_MLD_shelf = pd.concat([DF_GOFS_MLD_shelf, df_GOFS_MLD])
            DF_RTOFS_OHC_shelf = pd.concat([DF_RTOFS_OHC_shelf, df_RTOFS_OHC])
            DF_RTOFS_DA_OHC_shelf = pd.concat([DF_RTOFS_DA_OHC_shelf, df_RTOFS_DA_OHC])
            DF_GOFS_OHC_shelf = pd.concat([DF_GOFS_OHC_shelf, df_GOFS_OHC])
            DF_RTOFS_T100_shelf = pd.concat([DF_RTOFS_T100_shelf, df_RTOFS_T100])
            DF_RTOFS_DA_T100_shelf = pd.concat([DF_RTOFS_DA_T100_shelf, df_RTOFS_DA_T100])
            DF_GOFS_T100_shelf = pd.concat([DF_GOFS_T100_shelf, df_GOFS_T100])
            DF_RTOFS_T100_300_shelf = pd.concat([DF_RTOFS_T100_300_shelf, df_RTOFS_T100_300])
            DF_RTOFS_DA_T100_300_shelf = pd.concat([DF_RTOFS_DA_T100_300_shelf, df_RTOFS_DA_T100_300])
            DF_GOFS_T100_300_shelf = pd.concat([DF_GOFS_T100_300_shelf, df_GOFS_T100_300])
            DF_RTOFS_PEA_shelf = pd.concat([DF_RTOFS_PEA_shelf, df_RTOFS_PEA])
            DF_RTOFS_DA_PEA_shelf = pd.concat([DF_RTOFS_DA_PEA_shelf, df_RTOFS_DA_PEA])
            DF_GOFS_PEA_shelf = pd.concat([DF_GOFS_PEA_shelf, df_GOFS_PEA])

        if dataset_id.split('-')[0][0:2]=='cp':
            print('cp glider: '+dataset_id)
            #%% Concatenate data frames
            DF_RTOFS_temp_salt_cp = pd.concat([DF_RTOFS_temp_salt_cp, df_RTOFS_temp_salt])
            DF_RTOFS_DA_temp_salt_cp = pd.concat([DF_RTOFS_DA_temp_salt_cp, df_RTOFS_DA_temp_salt])
            DF_GOFS_temp_salt_cp = pd.concat([DF_GOFS_temp_salt_cp, df_GOFS_temp_salt])
            DF_RTOFS_MLD_cp = pd.concat([DF_RTOFS_MLD_cp, df_RTOFS_MLD])
            DF_RTOFS_DA_MLD_cp = pd.concat([DF_RTOFS_DA_MLD_cp, df_RTOFS_DA_MLD])
            DF_GOFS_MLD_cp = pd.concat([DF_GOFS_MLD_cp, df_GOFS_MLD])
            DF_RTOFS_OHC_cp = pd.concat([DF_RTOFS_OHC_cp, df_RTOFS_OHC])
            DF_RTOFS_DA_OHC_cp = pd.concat([DF_RTOFS_DA_OHC_cp, df_RTOFS_DA_OHC])
            DF_GOFS_OHC_cp = pd.concat([DF_GOFS_OHC_cp, df_GOFS_OHC])
            DF_RTOFS_T100_cp = pd.concat([DF_RTOFS_T100_cp, df_RTOFS_T100])
            DF_RTOFS_DA_T100_cp = pd.concat([DF_RTOFS_DA_T100_cp, df_RTOFS_DA_T100])
            DF_GOFS_T100_cp = pd.concat([DF_GOFS_T100_cp, df_GOFS_T100])
            DF_RTOFS_T100_300_cp = pd.concat([DF_RTOFS_T100_300_cp, df_RTOFS_T100_300])
            DF_RTOFS_DA_T100_300_cp = pd.concat([DF_RTOFS_DA_T100_300_cp, df_RTOFS_DA_T100_300])
            DF_GOFS_T100_300_cp = pd.concat([DF_GOFS_T100_300_cp, df_GOFS_T100_300])
            DF_RTOFS_PEA_cp = pd.concat([DF_RTOFS_PEA_cp, df_RTOFS_PEA])
            DF_RTOFS_DA_PEA_cp = pd.concat([DF_RTOFS_DA_PEA_cp, df_RTOFS_DA_PEA])
            DF_GOFS_PEA_cp = pd.concat([DF_GOFS_PEA_cp, df_GOFS_PEA])


        if dataset_id.split('-')[0][0:2]=='sp':
            print('sp glider: '+dataset_id)
            #%% Concatenate data frames
            DF_RTOFS_temp_salt_sp = pd.concat([DF_RTOFS_temp_salt_sp, df_RTOFS_temp_salt])
            DF_RTOFS_DA_temp_salt_sp = pd.concat([DF_RTOFS_DA_temp_salt_sp, df_RTOFS_DA_temp_salt])
            DF_GOFS_temp_salt_sp = pd.concat([DF_GOFS_temp_salt_sp, df_GOFS_temp_salt])
            DF_RTOFS_MLD_sp = pd.concat([DF_RTOFS_MLD_sp, df_RTOFS_MLD])
            DF_RTOFS_DA_MLD_sp = pd.concat([DF_RTOFS_DA_MLD_sp, df_RTOFS_DA_MLD])
            DF_GOFS_MLD_sp = pd.concat([DF_GOFS_MLD_sp, df_GOFS_MLD])
            DF_RTOFS_OHC_sp = pd.concat([DF_RTOFS_OHC_sp, df_RTOFS_OHC])
            DF_RTOFS_DA_OHC_sp = pd.concat([DF_RTOFS_DA_OHC_sp, df_RTOFS_DA_OHC])
            DF_GOFS_OHC_sp = pd.concat([DF_GOFS_OHC_sp, df_GOFS_OHC])
            DF_RTOFS_T100_sp = pd.concat([DF_RTOFS_T100_sp, df_RTOFS_T100])
            DF_RTOFS_DA_T100_sp = pd.concat([DF_RTOFS_DA_T100_sp, df_RTOFS_DA_T100])
            DF_GOFS_T100_sp = pd.concat([DF_GOFS_T100_sp, df_GOFS_T100])
            DF_RTOFS_T100_300_sp = pd.concat([DF_RTOFS_T100_300_sp, df_RTOFS_T100_300])
            DF_RTOFS_DA_T100_300_sp = pd.concat([DF_RTOFS_DA_T100_300_sp, df_RTOFS_DA_T100_300])
            DF_GOFS_T100_300_sp = pd.concat([DF_GOFS_T100_300_sp, df_GOFS_T100_300])
            DF_RTOFS_PEA_sp = pd.concat([DF_RTOFS_PEA_sp, df_RTOFS_PEA])
            DF_RTOFS_DA_PEA_sp = pd.concat([DF_RTOFS_DA_PEA_sp, df_RTOFS_DA_PEA])
            DF_GOFS_PEA_sp = pd.concat([DF_GOFS_PEA_sp, df_GOFS_PEA])


#%% Save all data frames
DF_RTOFS_temp_salt_shelf.to_pickle('DF_RTOFS_temp_salt_MAB_shelf.pkl')
DF_RTOFS_MLD_shelf.to_pickle('DF_RTOFS_MLD_MAB_shelf.pkl')
DF_RTOFS_OHC_shelf.to_pickle('DF_RTOFS_OHC_MAB_shelf.pkl')
DF_RTOFS_T100_shelf.to_pickle('DF_RTOFS_T100_MAB_shelf.pkl')
DF_RTOFS_T100_300_shelf.to_pickle('DF_RTOFS_T100_300_MAB.pkl')
DF_RTOFS_PEA_shelf.to_pickle('DF_RTOFS_PEA_MAB_shelf.pkl')

DF_RTOFS_DA_temp_salt_shelf.to_pickle('DF_RTOFS_DA_temp_salt_MAB_shelf.pkl')
DF_RTOFS_DA_MLD_shelf.to_pickle('DF_RTOFS_DA_MLD_MAB_shelf.pkl')
DF_RTOFS_DA_OHC_shelf.to_pickle('DF_RTOFS_DA_OHC_MAB_shelf.pkl')
DF_RTOFS_DA_T100_shelf.to_pickle('DF_RTOFS_DA_T100_MAB_shelf.pkl')
DF_RTOFS_DA_T100_300_shelf.to_pickle('DF_RTOFS_DA_T100_300_MAB_shelf.pkl')
DF_RTOFS_DA_PEA_shelf.to_pickle('DF_RTOFS_DA_PEA_MAB_shelf.pkl')

DF_GOFS_temp_salt_shelf.to_pickle('DF_GOFS_temp_salt_MAB_shelf.pkl')
DF_GOFS_MLD_shelf.to_pickle('DF_GOFS_MLD_MAB_shelf.pkl')
DF_GOFS_OHC_shelf.to_pickle('DF_GOFS_OHC_MAB_shelf.pkl')
DF_GOFS_T100_shelf.to_pickle('DF_GOFS_T100_MAB_shelf.pkl')
DF_GOFS_T100_300_shelf.to_pickle('DF_GOFS_T100_300_MAB_shelf.pkl')
DF_GOFS_PEA_shelf.to_pickle('DF_GOFS_PEA_MAB_shelf.pkl')

#%% Save all data frames
DF_RTOFS_temp_salt_cp.to_pickle('DF_RTOFS_temp_salt_MAB_cp.pkl')
DF_RTOFS_MLD_cp.to_pickle('DF_RTOFS_MLD_MAB_cp.pkl')
DF_RTOFS_OHC_cp.to_pickle('DF_RTOFS_OHC_MAB_cp.pkl')
DF_RTOFS_T100_cp.to_pickle('DF_RTOFS_T100_MAB_cp.pkl')
DF_RTOFS_T100_300_cp.to_pickle('DF_RTOFS_T100_300_MAB_cp.pkl')
DF_RTOFS_PEA_cp.to_pickle('DF_RTOFS_PEA_MAB_cp.pkl')

DF_RTOFS_DA_temp_salt_cp.to_pickle('DF_RTOFS_DA_temp_salt_MAB_cp.pkl')
DF_RTOFS_DA_MLD_cp.to_pickle('DF_RTOFS_DA_MLD_MAB_cp.pkl')
DF_RTOFS_DA_OHC_cp.to_pickle('DF_RTOFS_DA_OHC_MAB_cp.pkl')
DF_RTOFS_DA_T100_cp.to_pickle('DF_RTOFS_DA_T100_MAB_cp.pkl')
DF_RTOFS_DA_T100_300_cp.to_pickle('DF_RTOFS_DA_T100_300_MAB_cp.pkl')
DF_RTOFS_DA_PEA_cp.to_pickle('DF_RTOFS_DA_PEA_MAB_cp.pkl')

DF_GOFS_temp_salt_cp.to_pickle('DF_GOFS_temp_salt_MAB_cp.pkl')
DF_GOFS_MLD_cp.to_pickle('DF_GOFS_MLD_MAB_cp.pkl')
DF_GOFS_OHC_cp.to_pickle('DF_GOFS_OHC_MAB_cp.pkl')
DF_GOFS_T100_cp.to_pickle('DF_GOFS_T100_MAB_cp.pkl')
DF_GOFS_T100_300_cp.to_pickle('DF_GOFS_T100_300_MAB_cp.pkl')
DF_GOFS_PEA_cp.to_pickle('DF_GOFS_PEA_MAB_cp.pkl')

#%% Save all data frames
DF_RTOFS_temp_salt_sp.to_pickle('DF_RTOFS_temp_salt_MAB_sp.pkl')
DF_RTOFS_MLD_sp.to_pickle('DF_RTOFS_MLD_MAB_sp.pkl')
DF_RTOFS_OHC_sp.to_pickle('DF_RTOFS_OHC_MAB_sp.pkl')
DF_RTOFS_T100_sp.to_pickle('DF_RTOFS_T100_MAB_sp.pkl')
DF_RTOFS_T100_300_sp.to_pickle('DF_RTOFS_T100_300_MAB_sp.pkl')
DF_RTOFS_PEA_sp.to_pickle('DF_RTOFS_PEA_MAB_sp.pkl')

DF_RTOFS_DA_temp_salt_sp.to_pickle('DF_RTOFS_DA_temp_salt_MAB_sp.pkl')
DF_RTOFS_DA_MLD_sp.to_pickle('DF_RTOFS_DA_MLD_MAB_sp.pkl')
DF_RTOFS_DA_OHC_sp.to_pickle('DF_RTOFS_DA_OHC_MAB_sp.pkl')
DF_RTOFS_DA_T100_sp.to_pickle('DF_RTOFS_DA_T100_MAB_sp.pkl')
DF_RTOFS_DA_T100_300_sp.to_pickle('DF_RTOFS_DA_T100_300_MAB_sp.pkl')
DF_RTOFS_DA_PEA_sp.to_pickle('DF_RTOFS_DA_PEA_MAB_sp.pkl')

DF_GOFS_temp_salt_sp.to_pickle('DF_GOFS_temp_salt_MAB_sp.pkl')
DF_GOFS_MLD_sp.to_pickle('DF_GOFS_MLD_MAB_sp.pkl')
DF_GOFS_OHC_sp.to_pickle('DF_GOFS_OHC_MAB_sp.pkl')
DF_GOFS_T100_sp.to_pickle('DF_GOFS_T100_MAB_sp.pkl')
DF_GOFS_T100_300_sp.to_pickle('DF_GOFS_T100_300_MAB_sp.pkl')
DF_GOFS_PEA_sp.to_pickle('DF_GOFS_PEA_MAB_sp.pkl')

#%% Load all data frames
'''
DF_RTOFS_temp_salt_shelf = pd.read_pickle('DF_RTOFS_temp_salt_MAB_shelf.pkl')
DF_RTOFS_MLD_shelf = pd.read_pickle('DF_RTOFS_MLD_MAB_shelf.pkl')
DF_RTOFS_OHC_shelf = pd.read_pickle('DF_RTOFS_OHC_MAB_shelf.pkl')
DF_RTOFS_T100_shelf = pd.read_pickle('DF_RTOFS_T100_MAB_shelf.pkl')
DF_RTOFS_T100_300_shelf = pd.read_pickle('DF_RTOFS_T100_300_MAB_shelf.pkl')
DF_RTOFS_PEA_shelf = pd.read_pickle('DF_RTOFS_PEA_MAB_shelf.pkl')

DF_RTOFS_DA_temp_salt_shelf = pd.read_pickle('DF_RTOFS_DA_temp_salt_MAB_shelf.pkl')
DF_RTOFS_DA_MLD_shelf = pd.read_pickle('DF_RTOFS_DA_MLD_MAB_shelf.pkl')
DF_RTOFS_DA_OHC_shelf = pd.read_pickle('DF_RTOFS_DA_OHC_MAB_shelf.pkl')
DF_RTOFS_DA_T100_shelf = pd.read_pickle('DF_RTOFS_DA_T100_MAB_shelf.pkl')
DF_RTOFS_DA_T100_300_shelf = pd.read_pickle('DF_RTOFS_DA_T100_300_MAB_shelf.pkl')
DF_RTOFS_DA_PEA_shelf = pd.read_pickle('DF_RTOFS_DA_PEA_MAB_shelf.pkl')

DF_GOFS_temp_salt_shelf = pd.read_pickle('DF_GOFS_temp_salt_MAB_shelf.pkl')
DF_GOFS_MLD_shelf = pd.read_pickle('DF_GOFS_MLD_MAB_shelf.pkl')
DF_GOFS_OHC_shelf = pd.read_pickle('DF_GOFS_OHC_MAB_shelf.pkl')
DF_GOFS_T100_shelf = pd.read_pickle('DF_GOFS_T100_MAB_shelf.pkl')
DF_GOFS_T100_300_shelf = pd.read_pickle('DF_GOFS_T100_300_MAB_shelf.pkl')
DF_GOFS_PEA_shelf = pd.read_pickle('DF_GOFS_PEA_MAB_shelf.pkl')

##########
DF_RTOFS_temp_salt_sp = pd.read_pickle('DF_RTOFS_temp_salt_MAB_sp.pkl')
DF_RTOFS_MLD_sp = pd.read_pickle('DF_RTOFS_MLD_MAB_sp.pkl')
DF_RTOFS_OHC_sp = pd.read_pickle('DF_RTOFS_OHC_MAB_sp.pkl')
DF_RTOFS_T100_sp = pd.read_pickle('DF_RTOFS_T100_MAB_sp.pkl')
DF_RTOFS_T100_300_sp = pd.read_pickle('DF_RTOFS_T100_300_MAB_sp.pkl')
DF_RTOFS_PEA_sp = pd.read_pickle('DF_RTOFS_PEA_MAB_sp.pkl')

DF_RTOFS_DA_temp_salt_sp = pd.read_pickle('DF_RTOFS_DA_temp_salt_MAB_sp.pkl')
DF_RTOFS_DA_MLD_sp = pd.read_pickle('DF_RTOFS_DA_MLD_MAB_sp.pkl')
DF_RTOFS_DA_OHC_sp = pd.read_pickle('DF_RTOFS_DA_OHC_MAB_sp.pkl')
DF_RTOFS_DA_T100_sp = pd.read_pickle('DF_RTOFS_DA_T100_MAB_sp.pkl')
DF_RTOFS_DA_T100_300_sp = pd.read_pickle('DF_RTOFS_DA_T100_300_MAB_sp.pkl')
DF_RTOFS_DA_PEA_sp = pd.read_pickle('DF_RTOFS_DA_PEA_MAB_sp.pkl')

DF_GOFS_temp_salt_sp = pd.read_pickle('DF_GOFS_temp_salt_MAB_sp.pkl')
DF_GOFS_MLD_sp = pd.read_pickle('DF_GOFS_MLD_MAB_sp.pkl')
DF_GOFS_OHC_sp = pd.read_pickle('DF_GOFS_OHC_MAB_sp.pkl')
DF_GOFS_T100_sp = pd.read_pickle('DF_GOFS_T100_MAB_sp.pkl')
DF_GOFS_T100_300_sp = pd.read_pickle('DF_GOFS_T100_300_MAB_sp.pkl')
DF_GOFS_PEA_sp = pd.read_pickle('DF_GOFS_PEA_MAB_sp.pkl')

####################
DF_RTOFS_temp_salt_cp = pd.read_pickle('DF_RTOFS_temp_salt_MAB_cp.pkl')
DF_RTOFS_MLD_cp = pd.read_pickle('DF_RTOFS_MLD_MAB_cp.pkl')
DF_RTOFS_OHC_cp = pd.read_pickle('DF_RTOFS_OHC_MAB_cp.pkl')
DF_RTOFS_T100_cp = pd.read_pickle('DF_RTOFS_T100_MAB_cp.pkl')
DF_RTOFS_T100_300_cp = pd.read_pickle('DF_RTOFS_T100_300_MAB_cp.pkl')
DF_RTOFS_PEA_cp = pd.read_pickle('DF_RTOFS_PEA_MAB_cp.pkl')

DF_RTOFS_DA_temp_salt_cp = pd.read_pickle('DF_RTOFS_DA_temp_salt_MAB_cp.pkl')
DF_RTOFS_DA_MLD_cp = pd.read_pickle('DF_RTOFS_DA_MLD_MAB_cp.pkl')
DF_RTOFS_DA_OHC_cp = pd.read_pickle('DF_RTOFS_DA_OHC_MAB_cp.pkl')
DF_RTOFS_DA_T100_cp = pd.read_pickle('DF_RTOFS_DA_T100_MAB_cp.pkl')
DF_RTOFS_DA_T100_300_cp = pd.read_pickle('DF_RTOFS_DA_T100_300_MAB_cp.pkl')
DF_RTOFS_DA_PEA_cp = pd.read_pickle('DF_RTOFS_DA_PEA_MAB_cp.pkl')

DF_GOFS_temp_salt_cp = pd.read_pickle('DF_GOFS_temp_salt_MAB_cp.pkl')
DF_GOFS_MLD_cp = pd.read_pickle('DF_GOFS_MLD_MAB_cp.pkl')
DF_GOFS_OHC_cp = pd.read_pickle('DF_GOFS_OHC_MAB_cp.pkl')
DF_GOFS_T100_cp = pd.read_pickle('DF_GOFS_T100_MAB_cp.pkl')
DF_GOFS_T100_300_cp = pd.read_pickle('DF_GOFS_T100_300_MAB_cp.pkl')
DF_GOFS_PEA_cp = pd.read_pickle('DF_GOFS_PEA_MAB_cp.pkl')

'''

#################### Shelf gliders ###############################
#%% Temperature statistics.
DF_RTOFS = DF_RTOFS_temp_salt_shelf.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_temp_salt_shelf.dropna()
DF_GOFS = DF_GOFS_temp_salt_shelf.dropna()
...
DF_RTOFS = DF_RTOFS_MLD_shelf.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_MLD_shelf.dropna()
DF_GOFS = DF_GOFS_MLD_shelf.dropna()
...
#%% OHC statistics
DF_RTOFS = DF_RTOFS_OHC_shelf.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_OHC_shelf.dropna()
DF_GOFS = DF_GOFS_OHC_shelf.dropna()
...
#%% T100 statistics
DF_RTOFS = DF_RTOFS_T100_shelf.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_T100_shelf.dropna()
DF_GOFS = DF_GOFS_T100_shelf.dropna()
...
#%% T100_300 statistics
DF_RTOFS = DF_RTOFS_T100_300_shelf.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_T100_300_shelf.dropna()
DF_GOFS = DF_GOFS_T100_300_shelf.dropna()

DF_RTOFS = DF_RTOFS_T100_300_shelf.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_T100_300_shelf.dropna()
DF_GOFS = DF_GOFS_T100_300_shelf.dropna()

DF_RTOFS = DF_RTOFS[DF_RTOFS.T100_300_obs<100]
DF_RTOFS_DA = DF_RTOFS_DA[DF_RTOFS_DA.T100_300_obs<100]
DF_GOFS = DF_GOFS[DF_GOFS.T100_300_obs<100]
...
#CORR
tskill[0,0] = DF_RTOFS.corr()['T100_300_obs']['T100_300_RTOFS']
tskill[1,0] = DF_RTOFS_DA.corr()['T100_300_obs']['T100_300_RTOFS_DA']
tskill[2,0] = DF_GOFS.corr()['T100_300_obs']['T100_300_GOFS']

#OSTD
tskill[0,1] = DF_RTOFS.std().T100_300_obs
tskill[1,1] = DF_RTOFS_DA.std().T100_300_obs
tskill[2,1] = DF_GOFS.std().T100_300_obs

#MSTD
tskill[0,2] = DF_RTOFS.std().T100_300_RTOFS
tskill[1,2] = DF_RTOFS_DA.std().T100_300_RTOFS_DA
tskill[2,2] = DF_GOFS.std().T100_300_GOFS

#CRMSE
tskill[0,3] = np.sqrt(np.nansum(((DF_RTOFS.T100_300_obs-DF_RTOFS.mean().T100_300_obs)-\
                                 (DF_RTOFS.T100_300_RTOFS-DF_RTOFS.mean().T100_300_RTOFS))**2)/NRTOFS)
tskill[1,3] = np.sqrt(np.nansum(((DF_RTOFS_DA.T100_300_obs-DF_RTOFS_DA.mean().T100_300_obs)-\
                                 (DF_RTOFS_DA.T100_300_RTOFS_DA-DF_RTOFS_DA.mean().T100_300_RTOFS_DA))**2)/NRTOFS_DA)
tskill[2,3] = np.sqrt(np.nansum(((DF_GOFS.T100_300_obs-DF_GOFS.mean().T100_300_obs)-\
                                 (DF_GOFS.T100_300_GOFS-DF_GOFS.mean().T100_300_GOFS))**2)/NGOFS)

#BIAS
tskill[0,4] = DF_RTOFS.mean().T100_300_obs - DF_RTOFS.mean().T100_300_RTOFS
tskill[1,4] = DF_RTOFS_DA.mean().T100_300_obs - DF_RTOFS_DA.mean().T100_300_RTOFS_DA
tskill[2,4] = DF_GOFS.mean().T100_300_obs - DF_GOFS.mean().T100_300_GOFS

#color
colors = ['indianred','seagreen','darkorchid','darkorange']

T100_300_skillscores = pd.DataFrame(tskill,
                        index=['RTOFS','RTOFS_DA','GOFS'],
                        columns=cols)
print(T100_300_skillscores)

#%% PEA statistics
DF_RTOFS = DF_RTOFS_PEA_shelf.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_PEA_shelf.dropna()
DF_GOFS = DF_GOFS_PEA_shelf.dropna()
...
scores = T100_300_skillscores
for i,r in enumerate(scores.iterrows()):
    theta=np.arccos(r[1].CORRELATION)
    rr=r[1].MSTD/r[1].OSTD
    if i==2:
        ax1.plot(theta,rr,markers[i],label='T100_300',color = 'darkslateblue',alpha=0.7,markersize=8,markeredgecolor='r')
        ax1.plot(theta,rr,markers[i],fillstyle='none',markersize=8,markeredgecolor='r')
    else:
        ax1.plot(theta,rr,markers[i],color = 'darkslateblue',alpha=0.7,markersize=8,markeredgecolor='r')
        ax1.plot(theta,rr,markers[i],fillstyle='none',markersize=8,markeredgecolor='r')
...
file = folder_fig + 'Taylor_norm_RTOFS_RTOFS_DA_GOFS_MAB_shelf'
plt.savefig(file,bbox_inches = 'tight',pad_inches = 0.1)

#################### cp gliders ###############################
#%% Temperature statistics.
DF_RTOFS = DF_RTOFS_temp_salt_cp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_temp_salt_cp.dropna()
DF_GOFS = DF_GOFS_temp_salt_cp.dropna()
...
#%% Mixed layer statistics Temperature.
DF_RTOFS = DF_RTOFS_MLD_cp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_MLD_cp.dropna()
DF_GOFS = DF_GOFS_MLD_cp.dropna()
...
#%% OHC statistics
DF_RTOFS = DF_RTOFS_OHC_cp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_OHC_cp.dropna()
DF_GOFS = DF_GOFS_OHC_cp.dropna()
...
#%% T100 statistics
DF_RTOFS = DF_RTOFS_T100_cp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_T100_cp.dropna()
DF_GOFS = DF_GOFS_T100_cp.dropna()
...
#%% T100_300 statistics
DF_RTOFS = DF_RTOFS_T100_300_cp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_T100_300_cp.dropna()
DF_GOFS = DF_GOFS_T100_300_cp.dropna()
...
#%% PEA statistics
DF_RTOFS = DF_RTOFS_PEA_cp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_PEA_cp.dropna()
DF_GOFS = DF_GOFS_PEA_cp.dropna()
...
file = folder_fig + 'Taylor_norm_RTOFS_RTOFS_DA_GOFS_MAB_cp'
plt.savefig(file,bbox_inches = 'tight',pad_inches = 0.1)

#################### sp gliders ###############################
#%% Temperature statistics.
DF_RTOFS = DF_RTOFS_temp_salt_sp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_temp_salt_sp.dropna()
DF_GOFS = DF_GOFS_temp_salt_sp.dropna()
...
#%% Mixed layer statistics Temperature.
DF_RTOFS = DF_RTOFS_MLD_sp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_MLD_sp.dropna()
DF_GOFS = DF_GOFS_MLD_sp.dropna()
...
#%% OHC statistics
DF_RTOFS = DF_RTOFS_OHC_sp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_OHC_sp.dropna()
DF_GOFS = DF_GOFS_OHC_sp.dropna()
...
#%% T100 statistics
DF_RTOFS = DF_RTOFS_T100_sp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_T100_sp.dropna()
DF_GOFS = DF_GOFS_T100_sp.dropna()
...
#%% T100_300 statistics
DF_RTOFS = DF_RTOFS_T100_300_sp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_T100_300_sp.dropna()
DF_GOFS = DF_GOFS_T100_300_sp.dropna()
...
#%% PEA statistics
DF_RTOFS = DF_RTOFS_PEA_sp.dropna()
DF_RTOFS_DA = DF_RTOFS_DA_PEA_sp.dropna()
DF_GOFS = DF_GOFS_PEA_sp.dropna()
...
file = folder_fig + 'Taylor_norm_RTOFS_RTOFS_DA_GOFS_MAB_sp'
plt.savefig(file,bbox_inches = 'tight',pad_inches = 0.1)
